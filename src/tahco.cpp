...
Tahco::Tahco(StringVector* _patterns_ptr, size_t _num_trees){
...
void Tahco::verify_tree_building(){
    ...
    for(size_t iter = 0; iter < num_trees; ++iter){
        ...
        for(auto i = st; i < ed; ++i){
            //if(patterns[i]->length() > max_text_len){
            //    continue;
            //}
...
void Tahco::profile_patterns(const std::string &fp){
    StringVector &patterns = (*patterns_ptr);

    size_t num_patterns_per_tree = (num_patterns + num_trees - 1) / num_trees;
    
    //prescan(fp);
    //std::cerr << "prescanned" << std::endl;
    ...
    for(size_t iter = 0; iter < num_trees; ++iter){
        ...
        // build tahco
        build_tahco();
		time_construction += static_cast<double>(clock() - basic_start) / CLOCKS_PER_SEC;
        std::cerr << "Built tacho." << std::endl;
        
        basic_start = clock();
        profile_patterns_with_tahco(fp);
		time_query += static_cast<double>(clock() - basic_start) / CLOCKS_PER_SEC;
...
void Tahco::prescan(const std::string &fp){
    max_text_len = 0;
    ...
    while(auto bytes_read = ::read(fd, buf, BUFFER_SIZE)) {
        ...
        for(char *p = buf; (p = static_cast<char*>(std::memchr(p, '\n', static_cast<size_t>((buf + bytes_read) - p)))); ++p){            
            *p = '\0';
            if(line_no == 1){
                cur.append(st);
                max_text_len = std::max(max_text_len, cur.length());
                cur = std::string("");
            }
            st = p + 1;
            ++line_no;
...
void Tahco::profile_patterns_with_tahco(const std::string &fp){
...
void Tahco::add_pattern(std::string &pattern, size_t pattern_idx){
    ...
    /*
    if((*nodes[cur_node].hashmap)[cur_hash].size() > 0){
        std::cerr << (*nodes[cur_node].hashmap)[cur_hash][0] << " " << pattern_idx << std::endl;
    }
    */
    
    // (*nodes[cur_node].hashmap)[cur_hash].emplace_back(pattern_idx);
