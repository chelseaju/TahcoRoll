...
class Tahco{
    ...
    public:
        Tahco(StringVector* _patterns_ptr, size_t _num_trees);
        void initialize();
        void verify_tree_building();
        void profile_patterns(const std::string &fp);
        void prescan(const std::string &fp);
        void profile_patterns_with_tahco(const std::string &fp);
